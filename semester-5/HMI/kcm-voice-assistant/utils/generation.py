...
def generate_audio(text, prompt=None, language='auto', accent='no-accent'):
    ...
    # Preprocessing text and detecting language
    ...
    timings["text_preprocessing"] = time.time() - start_time

    start_time = time.time()
    if prompt is not None:
        prompt_path = prompt
        # print('it loaded my voice')
        ...
        audio_prompts = torch.tensor(prompt_data['audio_tokens']).type(torch.int32).to(device)
        text_prompts = torch.tensor(prompt_data['text_tokens']).type(torch.int32)
        lang_pr = code2lang[int(prompt_data['lang_code'])]
    else:
        # print('it didn\'t load my voice')
        audio_prompts = torch.zeros([1, 0, NUM_QUANTIZERS]).type(torch.int32).to(device)
        text_prompts = torch.zeros([1, 0]).type(torch.int32)
        lang_pr = lang if lang != 'mix' else 'en'
    enroll_x_lens = text_prompts.shape[-1]
    timings["prompt_loading"] = time.time() - start_time
    ...
    timings["text_tokenization"] = time.time() - start_time

    # VALL-E-X inference (generating audio tokens)
    start_time = time.time()
    encoded_frames = model.inference(
        ...
        top_k=0,  # Original: -100
        temperature=1,
        prompt_language=lang_pr,
        text_language=langs if accent == "no-accent" else lang,
    )
    timings["valle_inference"] = time.time() - start_time

    # Vocos decoding (audio token to waveform)
    start_time = time.time()
    frames = encoded_frames.permute(2, 0, 1)
    features = vocos.codes_to_features(frames)
    samples = vocos.decode(features, bandwidth_id=torch.tensor([1], device=device))
    timings["vocos_decoding"] = time.time() - start_time
