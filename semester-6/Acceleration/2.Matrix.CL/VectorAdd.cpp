      ...
      case 3: {
        ...
        while (sub_choice != 0) {
          ...
          if (sub_choice == 1) {
            ...
            for (cl_uint i = 0; i < total_devices; ++i) {
              cl_ulong max_len = get_max_vector_len(devices[i].device);
              cl_ulong actual_len = (user_len == 0 | user_len > max_len) ? max_len : user_len; 
              if (user_len > max_len) {
                printf("Device %d: requested size %zu too large, using max %zu instead", i, user_len, max_len);
              } else if (user_len % 64 != 0) {
                printf("Warning: vector length %zu is not divisible by local work-group size %zu.\n", user_len, local_item_size);
                printf("Consider rounding up to a multiple of 64 or use 0 for max.\n\n");
              }
...
void run_vector_add_on_device(cl_device_id dev, cl_ulong vector_len) {
  ...
  size_t max_vector_len = max_alloc / (3 * sizeof(int));
  if (max_vector_len > 32 * 1024 * 1024) {
    max_vector_len = 32 * 1024 * 1024;
  }
...
cl_ulong get_max_vector_len(cl_device_id dev) {
  cl_ulong max_alloc;
  clGetDeviceInfo(dev, CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(cl_ulong), &max_alloc, NULL);
