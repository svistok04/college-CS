...
zp=reshape(z,1,[]);
d=length(xp);
Z=[ones(1,d); xp; yp; xp.*yp; xp.^2; yp.^2];
C=zp*Z'*inv(Z*Z');
F2 = @(a, b) C(1) + C(2)*a + C(3)*b + C(4)*a.*b + C(5)*a.^2 + C(6)*b.^2;
z_test = F2(x_test, y_test)
mse = mean((z_real-z_test).^2);
disp(mse)
subplot(1, 2, 1);
plot3(X,Y,z,'.b', MarkerSize=10);
title('a) F1 w = c1 + c2*x + c3*y 9 samples MSE = ', num2str(mse));
hold on
W = F2(XX,YY);
mesh(XX,YY,W);
...
subplot(1, 2, 2);
plot3(X,Y,z,'.b', MarkerSize=10);
title('b) F2 w = c1 + c2*x + c3*y 42 samples MSE = ', num2str(mse));
hold on
W = F2(XX,YY);
mesh(XX,YY,W);
