...
void drawGraph(const DynamicArray<Vertex>*, const DynamicArray<Edge>*, const Edge*&, const std::string&);
...
int main() {
    ...
    // drawGraph(input1->getVertices(), input1->getEdges(), MST1, "g1.png");
    delete input1;
    delete[] MST1;
    ...
    // drawGraph(input2->getVertices(), input2->getEdges(), MST2, "g2.png");
    delete input2;
    delete[] MST2;
    ...
    // drawGraph(input3->getVertices(), input3->getEdges(), MST3, "g3.png");
    delete input3;
    delete[] MST3;
...
void drawGraph(const DynamicArray<Vertex>* graphVertices, const DynamicArray<Edge>* graphEdges, const Edge*& mstEdges, const std::string& filename) {
    cv::Mat image(1050, 1050, CV_8UC3, cv::Scalar(255, 255, 255));
    float scaleX = 1000.0f;
    float scaleY = 1000.0f;

    for (int i = 0; i < graphEdges->size; ++i) {
        cv::Point pt1(std::round(graphVertices->arr[graphEdges->arr[i].idSrc].x * scaleX),
                      std::round((1 - graphVertices->arr[graphEdges->arr[i].idSrc].y) * scaleY));
        cv::Point pt2(std::round(graphVertices->arr[graphEdges->arr[i].idDest].x * scaleX),
                      std::round((1 - graphVertices->arr[graphEdges->arr[i].idDest].y) * scaleY));
        cv::line(image, pt1, pt2, cv::Scalar(200, 200, 200));
    }

    for (int i = 0; i < graphVertices->size - 1; ++i) {
        cv::Point pt1(std::round(graphVertices->arr[mstEdges[i].idSrc].x * scaleX),
                      std::round((1 - graphVertices->arr[mstEdges[i].idSrc].y) * scaleY));
        cv::Point pt2(std::round(graphVertices->arr[mstEdges[i].idDest].x * scaleX),
                      std::round((1 - graphVertices->arr[mstEdges[i].idDest].y) * scaleY));
        cv::line(image, pt1, pt2, cv::Scalar(0, 0, 255));
    }

    for (int i = 0; i < graphVertices->size; ++i) {
        cv::Point center(std::round(graphVertices->arr[i].x * scaleX),
                         std::round((1 - graphVertices->arr[i].y) * scaleY));

        cv::circle(image, center, 5, cv::Scalar(0, 0, 0), -1);
        std::string vertexID = std::to_string(i);
        cv::Point textPos(center.x + 6, center.y - 6);
        cv::putText(image, vertexID, textPos, cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 0, 0), 1);
    }
