...
int main() {
    ...
    //
    // // auto* root= new Node<int>(0, distribution(gen));
    // RBT<int> myRBT;
    //
    ...
    // myRBT.insertNewNode(newNode);
    // std::cout << myRBT.RBT_to_string(1) << std::endl;
    // newNode = new Node<int>(11, 1);
    // myRBT.insertNewNode(newNode);
    // std::cout << myRBT.RBT_to_string(1) << std::endl;
    // newNode = new Node<int>(12, 70);
    // myRBT.insertNewNode(newNode);
    // std::cout << myRBT.RBT_to_string(1) << std::endl;
куыщшцrhoeifwweighorez;ewe;froguies
    // RBT<int> myRBT;
...
void RBT<T>::rightRotation(Node<T>*& node) {
    ...
    node->right = parent;
    node->parent = parent->parent;
    if (parent->parent) parent->parent->right = node;
    parent->parent = node;
    root->color = Black;
}
...
//         (grandparent->right == parent) ? grandparent->right = node : grandparent->left = node;
//     } else {
//         root = node;
//     }
