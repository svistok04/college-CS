...
//         for (int n = 0; n < N; ++n) {
//             f[n] = std::pow(n / static_cast<double>(N), 2);
//         }
//         t = clock();
...
//     }
    ...
    f[2] = 7;
    f[3] = 6;
    std::complex<double>* cFFT = fft(f, 4);
    delete[] f;
    return 0;
}
...
std::complex<double>* ftt_rec(double* input, int size, int step) {
    ...
    if (size == 1) {
        ...
        std::cout << size << " " << result[0] << std::endl;
    } else {
        std::complex<double>* even = ftt_rec(input, size / 2, 2 * step);
        std::complex<double>* odd = ftt_rec(input + step, size / 2, 2 * step);
        std::cout << size << " ";
        for (int k = 0; k < size/2; ++k) {
            std::complex E = even[k];
            double theta = -2* M_PI * k / size;
            std::complex<double> O = odd[k] * std::exp(theta * i);
            // std::complex<double> q = std::polar(1.0, -2 * M_PI * k / size) * odd[k];
            result[k] = E + O;
            result[k + size/2] = E - O;
            std::cout << "E is "<< even[k] << " " << "odd k is " << odd[k] << " " << "complex plane point is " << std::exp(theta * i) << " " << "O: " << " " << O << " " << "R" << k << " " << result[k] << " " << "R" << k+size/2 << " " <<  result[k + size/2] << " " << std::endl;
        }
        std::cout << std::endl;
        std::cout << std::endl;
        delete[] even;
